...
export const PayslipModal: React.FC<PayslipModalProps> = ({
  ...
  const currentPayslip = payrolls.find(
    (p) =>
      p.period === selectedPeriod && (p.user_id === user?.id || !p.user_id),
  ) ||
  ...
  const handleDownloadPdf = () => {
    Alert.alert(
      ...
      [
        { text: "Buka File", onPress: onClose },
        ...
        >
          ...
          >
            ...
            <View
              style={[
                styles.infoBox,
                isDark ? styles.infoBoxDark : styles.infoBoxLight,
              ]}
            >
              <Text style={styles.companyName}>BLUEHR ENTERPRISE CORP</Text>
              <Text style={styles.employeeName}>
                {user?.name || "Karyawan Perusahaan"}
              </Text>
              <Text style={styles.employeeSub}>
                {user?.position || "Senior Specialist"} •{" "}
                {user?.department || "Engineering"}
              </Text>
              <View style={styles.statusBadge}>
                <Feather name="check-circle" size={12} color="#059669" />
                <Text style={styles.statusBadgeText}>
                  TERBAYAR / PAID ({currentPayslip.period})
                </Text>
              </View>
            </View>
            ...
            <View style={styles.row}>
              ...
              <Text style={styles.rowVal}>
                {formatRupiah(currentPayslip.base_salary)}
              </Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.rowLabel}>Tunjangan Jabatan & Transport</Text>
              <Text style={styles.rowVal}>
                {formatRupiah(currentPayslip.allowance)}
              </Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.rowLabel}>Upah Lembur (Overtime)</Text>
              <Text style={styles.rowVal}>
                {formatRupiah(currentPayslip.overtime_pay)}
              </Text>
            </View>
            ...
            <View style={styles.row}>
              ...
              <Text style={[styles.rowVal, { color: "#e11d48" }]}>
                -
                {formatRupiah(
                  Math.round(currentPayslip.tax_bpjs_deduction * 0.5),
                )}
              </Text>
            </View>
            <View style={styles.row}>
            ...
            <View
              style={[
                styles.totalBox,
                isDark ? styles.totalBoxDark : styles.totalBoxLight,
              ]}
            >
              <Text style={styles.totalLabel}>TAKEOVER PAY (GAJI BERSIH)</Text>
              <Text style={styles.totalAmount}>
                {formatRupiah(currentPayslip.net_salary)}
              </Text>
            </View>
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  infoBox: { padding: 14, borderRadius: 5, marginBottom: 14 },
  infoBoxLight: {
