...
export const HolidayNoticeCard: React.FC<{ onOpenDetail?: () => void }> = ({
  onOpenDetail,
}) => {
  ...
  const latestAnn = announcements[0] || {
    ...
    content:
      "Seluruh karyawan diwajibkan melakukan absen geofencing radius 5 km dari lokasi kantor Jakarta Central.",
    category: "IMPORTANT",
  };
...
export const AttendanceTimelineWidget: React.FC = () => {
  ...
  return (
    <View style={[styles.card, isDark ? styles.cardDark : styles.cardLight]}>
        ...
        <View style={styles.leftTimeline}>
          ...
          <View style={styles.stepRow}>
            ...
            <View>
              ...
              <Text style={styles.stepLabel}>
                {t.checkInToday || "Check in Hari Ini"}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.stepRow}>
            ...
            <View>
              ...
              <Text style={styles.stepLabel}>
                {t.checkOutToday || "Check out Hari Ini"}
              </Text>
            </View>
          </View>
            ...
            <View>
              ...
              <Text style={styles.stepLabel}>
                ...
                <Text style={{ color: "#2563eb", fontWeight: "800" }}>
                  {todayAttendance?.check_out ? "+1h:30m" : "+0h:00m"}
                </Text>
              </Text>
            </View>
          </View>
          ...
          <View style={styles.totalHoursCard}>
            <View>
              <Text style={styles.hoursSub}>
                {t.totalWorkHours || "Total Jam Kerja"}
              </Text>
          ...
          <View style={styles.statusBox}>
            ...
            <View style={styles.greenPill}>
              ...
              <Text style={styles.greenPillText}>
                {todayAttendance ? "GPS Terverifikasi" : "Siap Clock In"}
              </Text>
            </View>
          </View>
        </View>
      ...
      <Text style={styles.shiftFooter}>
        Shift Kantor Standar (08:30 WIB - 17:30 WIB)
      </Text>
    </View>
  );
};
...
export const LeaveDonutSummaryCard: React.FC = () => {
  ...
  const annualUsed = approvedLeaves
    ...
    .reduce((acc, curr) => acc + (curr.duration_days || 1), 0);
    ...
    <View style={[styles.card, isDark ? styles.cardDark : styles.cardLight]}>
      ...
      <View style={styles.leaveHeaderRow}>
        ...
        <Text style={styles.availText}>
          Terpakai:{" "}
          <Text
            style={{ fontWeight: "900", color: isDark ? "#fff" : "#0f172a" }}
          >
            {annualUsed + sickUsed} Hari
          </Text>
        </Text>
      </View>
        ...
        <View style={styles.categoriesColumn}>
          <View style={styles.catRow}>
            ...
            <Text style={styles.catText}>
              Cuti Tahunan:{" "}
              <Text style={styles.boldText}>{annualUsed} Hari Dipakai</Text>
            </Text>
          </View>
          <View style={styles.catRow}>
            ...
            <Text style={styles.catText}>
              Cuti Sakit:{" "}
              <Text style={styles.boldText}>{sickUsed} Hari Dipakai</Text>
            </Text>
          </View>
          <View style={styles.catRow}>
...
export const WorkingHoursBarChartCard: React.FC = () => {
  ...
  const regularLeaveCount = leaves.filter(
    (l) => l.leave_type !== "SICK",
  ).length;
  ...
  const daysData = [
    ...
    },
    {
      day: "Rab",
      num: "27",
      type: regularLeaveCount > 0 ? "Leave" : "Kosong",
      label: regularLeaveCount > 0 ? "Cuti" : "Kosong",
      height: regularLeaveCount > 0 ? "65%" : "25%",
      bg: regularLeaveCount > 0 ? "#e9d5ff" : "rgba(15, 23, 42, 0.45)",
      badgeColor: regularLeaveCount > 0 ? "#a855f7" : "#94a3b8",
    },
    {
      day: "Kam",
      num: "28",
      type:
        sickLeaveCount > 0 ? "Sick" : todayAttendance ? "Present" : "Kosong",
      label:
        sickLeaveCount > 0 ? "Sakit" : todayAttendance ? "Hadir" : "Kosong",
      height: sickLeaveCount > 0 ? "55%" : todayAttendance ? "85%" : "25%",
      bg:
        sickLeaveCount > 0
          ? "#fbcfe8"
          : todayAttendance
    ...
    <View style={[styles.card, styles.blueChartCard]}>
      <View style={styles.widgetHeader}>
        <Text style={styles.blueCardTitle}>
          Statistik Jam Kerja (Working Hours)
        </Text>
            ...
            <View key={idx} style={styles.barCol}>
              <View style={styles.barTrack}>
                <View
                  style={[
                    styles.barFill,
                    { height: item.height as any, backgroundColor: item.bg },
                  ]}
                >
      ...
      <View style={styles.travelLeft}>
        ...
        <View>
          <Text style={styles.travelTitle}>
            Dinas Luar Kota (Travel on Duty)
          </Text>
          <Text style={styles.travelSub}>
            {user?.department
              ? `${user.department} • Penugasan Lapangan`
              : "Pengajuan Penugasan & Perjalanan Dinas"}
          </Text>
        </View>
      </View>
...
export const HRLeaveApprovalCard: React.FC = () => {
  ...
  const pendingLeave = leaves.find(
    (l) => l.status === "PENDING" || l.status === "APPROVED_L1",
  );
  ...
  return (
    ...
    >
      <View style={styles.approvalHeader}>
        <View style={styles.userAvatarBox}>
          <Text style={styles.avatarChar}>
            {pendingLeave.user_name?.substring(0, 1) || "H"}
          </Text>
        </View>
        <View style={{ flex: 1 }}>
          <Text
          ...
          >
            {pendingLeave.user_name || "Karyawan"}{" "}
            <Text style={styles.appliedText}>
              Pengajuan {pendingLeave.duration_days || 2} Hari
            </Text>
          </Text>
          ...
          <Text
            style={[styles.casualTag, { color: isL1 ? "#d97706" : "#2563eb" }]}
          >
            Status:{" "}
            {isL1
              ? "Menunggu Persetujuan L1 (Manager)"
              : "Disetujui L1 (Manager) - Menunggu Final HR"}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.approvalBtnRow}>
        ...
        >
          <Text style={styles.btnTextWhite}>{t.reject || "Reject"}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
    ...
    <View style={styles.teamRibbon}>
      <View style={styles.avatarOverlapRow}>
        {membersToDisplay.slice(0, 4).map((member, idx) => (
          <View
            key={member.id || idx}
            style={[styles.overlapAvatar, { marginLeft: idx > 0 ? -10 : 0 }]}
          >
      ...
      <View style={styles.memberBadge}>
        <Text style={styles.memberBadgeText}>
          {membersToDisplay.length} Anggota{" "}
          {user?.department ? user.department : "Departemen"} →
        </Text>
      </View>
    </View>
  );
...
// 0. Quick HR Actions Widget (Matches Web 6-tile Grid)
export const QuickHRActionsWidget: React.FC<{
  ...
  const allActions = [
    ...
    {
      key: "announcements",
      title: t.announcements || "Pengumuman",
      icon: "volume-2",
      color: "#d97706",
    ...
    {
      key: "reimbursements",
      title: t.reimbursements || "Reimbursement",
      icon: "credit-card",
      color: "#0891b2",
    ...
    >
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
        {actions.map((act) => (
          <TouchableOpacity
            key={act.key}
            onPress={() => onNavigate && onNavigate(act.key)}
            style={{
              width: "31%",
              paddingVertical: 10,
              paddingHorizontal: 4,
              ...
              justifyContent: "center",
              borderWidth: 1,
              borderColor: isDark ? "#1e293b" : "rgba(0,0,0,0.04)",
            }}
          >
            <View
              style={{
            ...
            >
              <Feather name={act.icon as any} size={16} color="#ffffff" />
            </View>
            <Text
              numberOfLines={1}
              style={{
...
// 10. Meeting Schedule Widget (Connected to Backend Schedules & Meeting Rooms)
export const MeetingScheduleWidget: React.FC = () => {
  const { schedules } = useHRStore();
  const { theme } = useThemeStore();
  const { t } = useLanguageStore();
  const isDark = theme === "dark";

  const upcomingMeeting = schedules[0] || {
    title: "Sprint Planning & Sync UI/UX",
    room_name: "Ruang Rapat Utama (Executive)",
  ...
  const getDeptColor = (tStr: string) => {
    ...
    if (
      ...
      return "#9333ea";
    if (s.includes("hr") || s.includes("people") || s.includes("capital"))
      return "#059669";
    if (s.includes("finance") || s.includes("tax") || s.includes("payroll"))
      return "#d97706";
    return "#e11d48";
  };

  const accentColor = getDeptColor(upcomingMeeting.title);

  const handleJoinMeeting = () => {
    if (upcomingMeeting.meeting_link) {
      Linking.openURL(upcomingMeeting.meeting_link);
    }
  };
    ...
    >
      ...
      >
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
          ...
          >
            {t.meetingSchedule || "Jadwal Rapat & Agenda"}
          </Text>
        </View>
        <TouchableOpacity
          style={[styles.allAppliedBtn, { backgroundColor: accentColor }]}
          onPress={handleJoinMeeting}
        >
          <Text style={styles.allAppliedText}>{t.joinMeet || "Join Meet"}</Text>
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        >
          {upcomingMeeting.room_name || "Ruang Rapat Utama"}
        </Text>
        <Text
          style={{
            fontSize: 13,
        ...
        <Text style={{ fontSize: 10, color: "#94a3b8", marginTop: 4 }}>
          {upcomingMeeting.date} • {upcomingMeeting.start_time} -{" "}
          {upcomingMeeting.end_time} ({upcomingMeeting.user_name})
        </Text>
      </View>
    </View>
  );
