...
export const PasswordStrengthChecklist: React.FC<{ password: string; isDark?: boolean }> = ({ password, isDark = false }) => {
  ...
  return (
    <View style={[styles.container, isDark ? styles.bgDark : styles.bgLight]}>
      <View style={styles.headerRow}>
        <Feather name="shield" size={14} color={rules.isValid ? '#10b981' : '#f59e0b'} />
        <Text style={[styles.headerTitle, isDark ? styles.textDark : styles.textLight]}>
          Kriteria Keamanan Password
        </Text>
        <View style={[styles.badge, rules.isValid ? styles.badgeGreen : styles.badgeAmber]}>
          <Text style={[styles.badgeText, rules.isValid ? styles.badgeTextGreen : styles.badgeTextAmber]}>
      ...
      {ruleItems.map((item, idx) => (
        <View key={idx} style={styles.itemRow}>
          <View style={[styles.iconCircle, item.met ? styles.iconCircleGreen : styles.iconCircleGrey]}>
            <Feather name={item.met ? 'check' : 'x'} size={10} color="#ffffff" />
          </View>
          <Text style={[styles.itemText, item.met ? styles.itemTextGreen : styles.itemTextGrey]}>
            {item.label}
          </Text>
        </View>
      ))}
...
const styles = StyleSheet.create({
  container: { padding: 12, borderRadius: 16, borderBottomWidth: 1, borderColor: '#e2e8f0', marginTop: 8 },
  ...
  itemRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginVertical: 2 },
  iconCircle: { width: 14, height: 14, borderRadius: 7, justifyContent: 'center', alignItems: 'center' },
