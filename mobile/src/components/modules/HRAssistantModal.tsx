...
export const HRAssistantModal: React.FC<HRAssistantModalProps> = ({
  ...
  >([
    {
      sender: "bot",
      text: "Halo! Saya Asisten HR AI. Ada yang bisa saya bantu terkait kebijakan cuti, klaim reimbursement, atau syarat lembur?",
    },
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      let reply =
        "Kuota cuti tahunan Anda saat ini tersisa 12 hari. Anda dapat mengajukan via menu Cuti di aplikasi.";
      if (
        userMsg.toLowerCase().includes("reimburse") ||
        userMsg.toLowerCase().includes("klaim")
      ) {
        reply =
          "Klaim reimbursement memerlukan upload foto nota/struk pengeluaran fisik yang valid melalui menu Reimbursements.";
      } else if (userMsg.toLowerCase().includes("lembur")) {
        reply =
          "Upah lembur dihitung 1.5x upah per jam untuk jam pertama dan 2x upah per jam untuk jam berikutnya.";
      }
...
const styles = StyleSheet.create({
  ...
  chatContent: { gap: 10 },
  msgBubble: { maxWidth: "80%", padding: 12, borderRadius: 5 },
