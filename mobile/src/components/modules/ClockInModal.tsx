...
export const ClockInModal: React.FC<ClockInModalProps> = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      const bioRes = await authenticateMobileBiometric(
        "Clock In Presensi Geofencing",
      );
      if (!bioRes.success) {
      ...
      await clockIn(
        ...
        notes || "Presensi Mobile (Biometric Verified)",
      );
      onClose();
    } catch (e) {
      ...
      <View style={styles.overlay}>
        ...
        >
          ...
          <View
            style={[
              styles.statusBox,
              inRadius ? styles.bgGreen : styles.bgRose,
            ]}
          >
            <Feather
              name={inRadius ? "check-circle" : "alert-circle"}
              size={18}
              color={inRadius ? "#22c55e" : "#f43f5e"}
            />
            <View style={{ flex: 1 }}>
              <Text
                style={[
              ...
              <Text style={styles.statusDesc}>
                Jarak GPS: {simulatedDistanceKm.toFixed(1)} km dari titik
                kantor.
              </Text>
            </View>
          </View>
