...
export const ShiftSwapScreen: React.FC = () => {
  ...
  const fetchSwaps = async () => {
    try {
      ...
      setSwaps([
        { id: 1, target_user_name: 'Budi Santoso', shift_date: '2026-08-28', status: 'PENDING', reason: 'Acara Keluarga' }
      ]);
    }
  };
  ...
  const handleSubmit = async () => {
    if (!targetUserId || !reason) {
      Alert.alert('Perhatian', 'Mohon isi ID rekan kerja pengganti dan alasan.');
      return;
    }
  ...
  return (
    <ScrollView style={[styles.container, isDark ? styles.bgDark : styles.bgLight]} contentContainerStyle={styles.content}>
      <View style={[styles.card, isDark ? styles.cardDark : styles.cardLight]}>
        <Text style={[styles.cardTitle, isDark ? styles.textDark : styles.textLight]}>{t.shiftSwap || 'Tukar Shift'}</Text>
        ...
        <TouchableOpacity style={styles.submitBtn} onPress={handleSubmit} disabled={isSubmitting}>
          <Text style={styles.submitBtnText}>{isSubmitting ? (t.submitting || 'Mengirim...') : 'Kirim Permohonan Tukar Shift'}</Text>
        </TouchableOpacity>
      </View>

      <Text style={[styles.sectionHeader, isDark ? styles.textDark : styles.textLight]}>Riwayat Permohonan Tukar Shift</Text>
      {swaps.map((item) => (
        <View key={item.id} style={[styles.historyCard, isDark ? styles.cardDark : styles.cardLight]}>
          <View style={{ flex: 1 }}>
            <Text style={[styles.historyTitle, isDark ? styles.textDark : styles.textLight]}>Shift {item.shift_date} • Rekan: {item.target_user_name || item.target_user_id}</Text>
