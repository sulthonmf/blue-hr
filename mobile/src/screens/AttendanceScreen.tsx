        ...
        attendanceLogs.map((item) => (
          <View key={item.id} style={[styles.card, isDark ? styles.cardDark : styles.cardLight]}>
            <View style={styles.row}>
              ...
              <View style={[styles.badge, item.status === 'ON_TIME' ? styles.bgGreen : styles.bgRose]}>
                <Text style={[styles.badgeText, item.status === 'ON_TIME' ? styles.textGreen : styles.textRose]}>
                  {item.status}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.footerRow}>
              <Text style={styles.distText}>Jarak: {item.distance_km ?? 1.2} km</Text>
              <Text style={styles.notesText}>{item.notes || 'Validated Geofence'}</Text>
            </View>
          </View>
        ))
      )}
...
const styles = StyleSheet.create({
  ...
  empty: { textAlign: 'center', color: '#94a3b8', fontSize: 12, marginVertical: 40 },
  card: { padding: 14, borderRadius: 20, marginBottom: 10, borderWidth: 1 },
